...
const countNumberOfElements = (arr) => {
  // Solution code here...
  let countedItems = 0;
  arr.reduce(value => countedItems ++, 'banana')
  return countedItems
};
...
const returnNames = (arr) => {
  // Solution code here...
  return arr.reduce((answerSoFar, person) => {
...
const reversedString = (str) => {
  ...
  let arr = str.split('');
  let newarr = [];
  arr.reduce((responseSoFar, banana) => newarr.unshift(banana), 'something');
  return newarr.join('');
};
...
const countNumberOfChildren = (arr) => {
  // Solution code here...
  return arr.reduce((resultSoFar, char) =>
    resultSoFar + (char.children === undefined ? 0 : char.children.length), 0);
};
...
const calculateAverage = (arr) => {
  ...
  const result = arr.reduce(
    (resultSoFar, num) => {
      return {sum: resultSoFar.sum + num, count: resultSoFar.count +1}},
    {sum: 0, count:0}
  );
